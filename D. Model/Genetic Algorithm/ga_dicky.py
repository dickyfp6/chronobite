...
def fitness(solution: pd.DataFrame, guidelines: Dict, tdee: Optional[float] = None) -> float:
    ...
    if tdee and tdee > 0:
        current_energy = total_nutrition.get('energy_kcal', 0)
        min_energy = 0.95 * tdee
        max_energy = 1.05 * tdee
...
def calculate_total_nutrition_numpy(
    ...
    if tdee is not None and tdee > 0:
        ...
        # Target energies per slot based on ratios
        target_energy_ratios = np.array([
            0.11875, 0.07125, 0.0475,  # Breakfast (Main, Side, Drink)
            0.16875, 0.10125, 0.0675,  # Lunch (Main, Side, Drink)
            0.14375, 0.08625, 0.0575,  # Dinner (Main, Side, Drink)
            0.1375                     # Snack
        ], dtype=np.float64)
        target_energies = tdee * target_energy_ratios
        
        # Calculate raw portion grams to hit target energy
        grams = (target_energies / food_energies) * 100.0
        
        # Clamp portions to realistic limits
        ...
        # Apply protein-based portion limiting if protein column is present
...
def run_ga_numpy(
    ...
    best_solution = indices_to_dataframe(best_individual, food_df_reset, tdee, food_matrix, nutrient_cols)
...
def local_search(
    ...
    # Pre-build lookup dict: food_name -> positional index (first occurrence)
    food_name_to_idx = {}
    for idx, name in enumerate(food_df_reset['food_name']):
        if name not in food_name_to_idx:
            food_name_to_idx[name] = idx
    
    def _solution_to_indices(solution_df: pd.DataFrame, food_df_reset: pd.DataFrame) -> np.ndarray:
        indices = []
        for i in range(len(solution_df)):
            food_name = solution_df.iloc[i].get('food_name', '')
            idx = food_name_to_idx.get(food_name)
            if idx is not None:
                indices.append(idx)
            else:
                print(f"[WARNING] Food name '{food_name}' not found in lookup dict, fallback to index 0")
                indices.append(0)
        return np.array(indices, dtype=np.int64)
    ...
    while iteration < iterations:
        ...
        for gene_idx in range(CHROMOSOME_SIZE):
            ...
            for idx_cand in range(min(15, len(slot_candidates))):
                ...
                food_name = new_food.get('food_name', '')
                new_food_idx_pos = food_name_to_idx.get(food_name)
