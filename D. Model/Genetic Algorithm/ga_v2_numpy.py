...
def fitness_numpy(
    ...
    """
    # Get total nutrition using numpy matrix
    total_nutrition = calculate_total_nutrition_numpy(indices, food_matrix, nutrient_cols)
    ...
    for nutrient_name, constraint in hard_constraints.items():
        ...
        # Tolerance: 80% - 120% (same as fitness())
        min_tolerance = 0.8 * min_val
        max_tolerance = 1.2 * max_val
...
def calculate_total_nutrition_numpy(
    ...
    This is the fast version that replaces calculate_total_nutrition() when using
    numpy-based individual representation.
    ...
    Returns:
        Dict: {nutrient_name: total_value, ...}
        Example: {'energy_kcal': 1850.5, 'protein_g': 65.2, 'fat_g': 45.1, ...}
    
    Performance:
    - O(n_selected) = O(10) constant time for 10-item meal plan
    - ~100x faster than DataFrame.sum() on 10 rows
    
    Example:
        >>> nutrition = calculate_total_nutrition_numpy(
        ...     indices=np.array([0, 5, 12, 3, 8, 15, 2, 7, 10, 4]),
        ...     food_matrix=food_matrix,
        ...     nutrient_cols=nutrient_cols
        ... )
        >>> print(nutrition['energy_kcal'])
        1850.5
    """
    # Select rows from food_matrix using indices, sum across axis 0
    selected_nutrients = food_matrix[indices].sum(axis=0)
    
    # Convert to dict: {nutrient_name: total_value}
    total_nutrition = dict(zip(nutrient_cols, selected_nutrients))
...
def indices_to_dataframe(indices: np.ndarray, food_df_reset: pd.DataFrame) -> pd.DataFrame:
    """
    Convert numpy index array to DataFrame solution
    ...
    result = food_df_reset.iloc[indices.astype(int)].reset_index(drop=True)
...
def run_ga_numpy(
    ...
    best_solution = indices_to_dataframe(best_individual, food_df_reset)
    ...
    top_solutions_df = [indices_to_dataframe(ind, food_df_reset) for ind in top_solutions]
