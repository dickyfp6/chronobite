...
def validate_final_solution(solution: pd.DataFrame, guidelines: Dict, tdee: Optional[float] = None) -> Dict:
    ...
    if 'final_energy_kcal' in solution.columns:
        total_nutrition = calculate_total_nutrition_from_portions(solution)
    else:
        raise ValueError("validate_final_solution dipanggil sebelum portioning selesai. "
                        "Pastikan indices_to_dataframe dipanggil dengan food_matrix dan nutrient_cols.")    
...
# CATATAN: Konstanta ini didefinisikan untuk weighted nutrient scoring
# namun saat ini tidak dipanggil di fitness_numpy() maupun fitness().
# Dipertahankan sebagai referensi untuk pengembangan di masa depan.
NUTRIENT_WEIGHTS = {
...
# ═════════════════════════════════════════════════════════════════════════════
# 1.5 NUMPY VERSIONS - Integer array representation (PERFORMANCE OPTIMIZATION)
# ═════════════════════════════════════════════════════════════════════════════

def random_solution_numpy(food_df: pd.DataFrame) -> np.ndarray:
    """
    Generate 1 numpy-based solution = array of 10 food indices (random)
    
    NUMPY VERSION - Returns integer indices instead of DataFrame
    Enables fast matrix lookups in GA loop
    
    Args:
        food_df: DataFrame berisi semua food items (with reset index for consistency)
    
    Returns:
        np.ndarray: shape (10,), dtype int
        Array of valid food indices, one per slot (0-9)
    
    Example:
        >>> indices = random_solution_numpy(food_df)
        >>> indices.shape
        (10,)
        >>> nutrition = calculate_total_nutrition_numpy(indices, food_matrix, nutrient_cols)
    
    Logic:
        - For each slot 0-9, filter valid foods by consumption_label using _filter_food_by_slot()
        - Random sample 1 food from filtered options
        - Get positional index in food_df using food_df.index.get_loc()
        - Collect 10 indices into result array
    """
    indices = []
    used_foods = set()
    
    for slot_idx in range(CHROMOSOME_SIZE):
        # Get valid foods for this slot
        slot_filtered = _filter_food_by_slot(food_df, slot_idx)
        
        if len(slot_filtered) == 0:
            # Fallback: use all foods if slot filter returns empty
            slot_filtered = food_df
        
        # Try to avoid duplicates if possible
        available = slot_filtered
        if 'food_name' in slot_filtered.columns and len(used_foods) > 0:
            not_used = slot_filtered[~slot_filtered['food_name'].isin(used_foods)]
            if len(not_used) > 0:
                available = not_used
        
        # Sample 1 food randomly
        selected_row = available.sample(n=1)
        
        # Get positional index in food_df
        food_idx_label = selected_row.index[0]  # Label-based index from food_df
        food_idx_pos = food_df.index.get_loc(food_idx_label)  # Positional index
        
        indices.append(food_idx_pos)
        
        # Track used foods for deduplication attempt
        if 'food_name' in selected_row.columns:
            food_name = selected_row.iloc[0].get('food_name', '')
            if food_name:
                used_foods.add(food_name)
...
def guided_solution_numpy(food_df: pd.DataFrame, guidelines: Dict) -> np.ndarray:
    """
    Generate 1 numpy-based solution = array of 10 food indices (macro-guided)
    
    NUMPY VERSION - Returns integer indices instead of DataFrame
    Uses same macro-aware selection logic as guided_solution()
    
    Args:
        food_df: DataFrame berisi semua food items (with reset index for consistency)
        guidelines: {'hard': {...}, 'soft': {...}} constraint dict
    
    Returns:
        np.ndarray: shape (10,), dtype int
        Array of food indices guided toward macro targets, one per slot
    
    Example:
        >>> indices = guided_solution_numpy(food_df, guidelines)
        >>> nutrition = calculate_total_nutrition_numpy(indices, food_matrix, nutrient_cols)
    
    Logic:
        - Extract macro targets from guidelines['hard']
        - For each slot 0-9:
          - Filter valid foods by consumption_label
          - Score candidates based on contribution to remaining macro needs
          - Select from top-20 scored foods
          - Update remaining nutrient requirements
        - Collect 10 food indices into result array
    """
    # LIMITASI: Scoring pemilihan makanan menggunakan nilai nutrisi @100g,
    # bukan nilai per porsi aktual. Makanan yang terlihat optimal @100g
    # belum tentu optimal setelah diporsi. Dampaknya terbatas karena
    # guided_solution hanya dipakai sebagai starting point populasi awal.
    # Extract macro targets from guidelines
    hard_constraints = guidelines.get('hard', {})
    ...
    for slot_idx in range(CHROMOSOME_SIZE):
        ...
        # Compute macro contribution scores
        slot_label = SLOT_LABEL_MAP.get(slot_idx, 'Main Course')
        scored_candidates = slot_candidates.copy()
        ...
        if 'carbohydrate_g' in scored_candidates.columns and \
           ...
           'fat_g' in scored_candidates.columns:
            ...
            # Calculate how much of each macro has been consumed
            protein_already_consumed = target_protein - remaining_protein
            carb_already_consumed = target_carb - remaining_carb
            fat_already_consumed = target_fat - remaining_fat
        ...
        # Select from top-20 scored candidates
        top_candidates = scored_candidates.nlargest(min(20, len(scored_candidates)), '_score')
        
        if len(top_candidates) > 0:
            selected_row = top_candidates.sample(n=1)
        else:
            selected_row = slot_candidates.sample(n=1)
        
        # Get positional index in food_df
        food_idx_label = selected_row.index[0]
        food_idx_pos = food_df.index.get_loc(food_idx_label)
...
def calculate_total_nutrition(solution: pd.DataFrame) -> Dict[str, float]:
    ...
    """
    if 'final_energy_kcal' in solution.columns:
        return calculate_total_nutrition_from_portions(solution)
    ...
    # Sum specific numeric columns to avoid expensive select_dtypes operation, filtering out final_ columns if present but not final_energy_kcal
    cols_to_sum = [c for c in _numeric_cols_cache if not c.startswith('final_') and c != 'gram' and c in solution.columns]
    total_nutrition = solution[cols_to_sum].sum().to_dict()
...
def is_feasible(solution: pd.DataFrame, guidelines: Dict) -> Tuple[bool, List[Dict]]:
    ...
    """
    # CATATAN: Fungsi ini tidak dipanggil di jalur produksi (run_ga_numpy).
    # Dipertahankan sebagai utility function untuk keperluan debugging
...
# LEGACY - Tidak dipakai di produksi. Jalur produksi pakai fitness_numpy().
# Dipertahankan untuk kompatibilitas mundur dan debugging.
...
def fitness(solution: pd.DataFrame, guidelines: Dict, tdee: Optional[float] = None) -> float:
    ...
    """
    if tdee and tdee > 0 and 'final_energy_kcal' not in solution.columns:
        solution_portioned = calculate_portion_sizes_dynamic(solution, tdee, guidelines)
        total_nutrition = calculate_total_nutrition(solution_portioned)
    else:
        total_nutrition = calculate_total_nutrition(solution)
    ...
    for nutrient_name, constraint in hard_constraints.items():
        ...
        # Tolerance: 100% strict (matches final validation)
        min_tolerance = min_val
    ...
    # energy_penalty: deviation of daily calories from TDEE
    energy_penalty = 0.0
    if tdee and tdee > 0:
        current_energy = total_nutrition.get('energy_kcal', 0)
        energy_constraint = hard_constraints.get('energy_kcal', {})
        min_energy = energy_constraint.get('min') or 0
        max_energy = energy_constraint.get('max') or float('inf')
        if current_energy < min_energy:
            energy_penalty = (min_energy - current_energy) * 50.0
        elif current_energy > max_energy:
            energy_penalty = (current_energy - max_energy) * 50.0
    ...
    total_penalty = (
        ...
        energy_penalty                # ENERGY: Target TDEE deviation penalty
    )
...
# ═════════════════════════════════════════════════════════════════════════════
# 3.25 FITNESS_NUMPY - Numpy-based fitness evaluation (PERFORMANCE OPTIMIZATION)
# ═════════════════════════════════════════════════════════════════════════════
...
def fitness_numpy(
    ...
    """
    Numpy version of fitness() - same penalty formula, faster execution
    
    Uses pre-built food_matrix for O(10) evaluation instead of DataFrame operations.
    Logic is identical to fitness() except for duplicate detection via indices.
    
    Args:
        indices: np.ndarray shape (10,) of food indices (integer array)
        food_matrix: Pre-built nutrient matrix, shape (n_foods, n_nutrients)
        nutrient_cols: List of nutrient column names (in same order as food_matrix)
        guidelines: {'hard': {...}, 'soft': {...}} constraint dict
        tdee: Target daily energy expenditure (kcal) - optional TDEE scaling
    
    Returns:
        float: Total penalty (lower = better, same units as fitness())
    
    Formula:
        total_penalty = (macro_penalty * 5000) + (hard_penalty * 10000) 
                      + (soft_penalty * 100) + diversity_penalty
    
    Performance:
    - O(10) constant time for nutrition calculation
    - ~10-100x faster than fitness() for large evaluations
    """
    # Get total nutrition using numpy matrix with dynamic portioning
    total_nutrition = calculate_total_nutrition_numpy(indices, food_matrix, nutrient_cols, tdee)
    
    # Dynamic portioning already scales nutrition to TDEE, so QUICK-SCALE is bypassed
    # ─────────────────────────────────────────────────────────────────────────────
    ...
    # ════════════════════════════════════════════════════════════════════════
    # CALCULATE MACRO PENALTY (dominant)
    # ════════════════════════════════════════════════════════════════════════
    # [ASSUMPTION DOC] Gap Bridge: If min > max (e.g., due to disease conflicts), 
    # data_loader.py handles it via merge_disease_guidelines() 'Closest Gap Bridge' strategy.
    # Therefore, min_val <= max_val is guaranteed here.
    ...
    for nutrient in macro_nutrients:
        ...
        # Calculate deviation (same formula as fitness())
        if actual < min_val:
            # LOW: how far below target
            deviation = (min_val - actual) / (min_val + 1)
        elif actual > max_val:
            # HIGH: how far above target
            deviation = (actual - max_val) / (max_val + 1)
        else:
    ...
    # ════════════════════════════════════════════════════════════════════════
    # DIVERSITY PENALTY (prevent food index repetition)
    # ════════════════════════════════════════════════════════════════════════
    # For numpy version: check duplicate indices instead of duplicate food_names
    # diversity_penalty += (num_duplicates) * DUPLICATE_PENALTY_WEIGHT
    
    diversity_penalty = 0.0
    
    # Count unique indices: if < 10, we have duplicates
    unique_count = len(set(indices.tolist()))
    num_duplicates = len(indices) - unique_count
...
# ═════════════════════════════════════════════════════════════════════════════
# 5.25 CROSSOVER_NUMPY & MUTATION_NUMPY - Numpy-based GA operators
# ═════════════════════════════════════════════════════════════════════════════

def crossover_numpy(parent1: np.ndarray, parent2: np.ndarray) -> np.ndarray:
    """
    Single-point crossover on numpy index arrays (NUMPY OPTIMIZATION)
    
    Creates a child individual by combining genes from two parent individuals
    represented as integer index arrays.
    
    Args:
        parent1: np.ndarray shape (10,) of food indices (dtype int)
        parent2: np.ndarray shape (10,) of food indices (dtype int)
    
    Returns:
        np.ndarray: shape (10,) child individual with genes from both parents
    
    Example:
        >>> parent1 = np.array([0, 5, 12, 3, 8, 15, 2, 7, 10, 4])
        >>> parent2 = np.array([1, 6, 13, 2, 7, 14, 3, 8, 11, 5])
        >>> 
        >>> # If crossover point = 3:
        >>> # child = [0, 5, 12] + [2, 7, 14, 3, 8, 11, 5]
    ...
    Logic:
        ...
        - Maintains genetic diversity by combining both parents
    """
    # CATATAN DESAIN: Menggunakan single-point crossover.
    # Gen 0-N dari parent1 dan gen N+1 sampai akhir dari parent2.
    # Konsekuensi: kombinasi antar meal slot (misal breakfast parent1 +
    # dinner parent2) tidak selalu terjadi. Pengembangan: uniform crossover
    # dapat meningkatkan eksplorasi kombinasi antar slot.
    # Ensure both parents have correct size
...
def mutation_numpy(
    ...
    mutation_rate: float = 0.3,
    guidelines: Optional[Dict] = None,
    food_matrix: Optional[np.ndarray] = None,
    nutrient_cols: Optional[List[str]] = None,
    tdee: Optional[float] = None
) -> np.ndarray:
    """
    Guided mutation on numpy index array (NUMPY OPTIMIZATION)
    
    Intelligently mutates 2-3 genes based on deficient macronutrients.
    Uses numpy-based nutrition calculation if food_matrix provided, else falls back to DataFrame.
    
    Args:
        indices: np.ndarray shape (10,) of food indices (dtype int)
        food_df: DataFrame with all food items (with reset index)
        mutation_rate: float probability (default 0.3 = 30% chance to mutate)
        guidelines: Optional {'hard': {...}, 'soft': {...}} constraints
        food_matrix: Optional pre-built nutrient matrix for fast calculation
        nutrient_cols: Optional nutrient column names (required if food_matrix provided)
        tdee: Optional target daily energy expenditure
    
    Returns:
        np.ndarray: shape (10,) mutated individual (or copy if no mutation)
    
    Logic:
        1. If random() > mutation_rate: return copy (no mutation)
        2. 20% chance: pure random mutation (skip guided filtering)
        3. Else: nutrient-aware mutation based on deficits
           - NEED_CARB: seek carb >= 20 AND protein <= 15
           - NEED_FAT: seek fat >= 10 AND protein <= 15
           - TOO_PROTEIN: seek protein <= 10
        4. Mutate 2-3 random slots using guided selection
    
    Performance:
    - O(10) if food_matrix provided (fast)
    - O(CHROMOSOME_SIZE * n_foods) if fallback to DataFrame sum
    """
    # CATATAN: Nilai default target_carb_min, target_fat_min, target_protein_max
    # di bawah menggunakan angka hardcoded sebagai fallback awal.
    # Nilai ini akan di-override oleh guidelines aktual jika tersedia.
    # Untuk kasus penyakit dengan constraint ketat (misal CKD: protein_max=40g),
    # pastikan guidelines selalu dipass dengan benar ke fungsi ini.
    ...
    # 20% chance: pure random mutation (full exploration)
    use_pure_random = random.random() < 0.2
    
    # Calculate current nutrient totals
    if food_matrix is not None and nutrient_cols is not None:
        # Fast numpy-based calculation
        total_nutrition = calculate_total_nutrition_numpy(indices, food_matrix, nutrient_cols, tdee)
        total_carb = total_nutrition.get('carbohydrate_g', 0)
        total_fat = total_nutrition.get('fat_g', 0)
        total_protein = total_nutrition.get('protein_g', 0)
    else:
        # Fallback: sum from food_df using indices
        selected_foods = food_df.iloc[indices]
        total_carb = selected_foods['carbohydrate_g'].sum() if 'carbohydrate_g' in selected_foods.columns else 0
        total_fat = selected_foods['fat_g'].sum() if 'fat_g' in selected_foods.columns else 0
        total_protein = selected_foods['protein_g'].sum() if 'protein_g' in selected_foods.columns else 0
    
    # Extract target values from guidelines or use defaults
    target_carb_min = 200
    target_fat_min = 50
    target_protein_max = 120
    ...
    # Mutate 2-3 random genes for diversity
    num_mutations = random.randint(2, 3)
    genes_to_mutate = random.sample(range(CHROMOSOME_SIZE), min(num_mutations, CHROMOSOME_SIZE))
    ...
    for gene_idx in genes_to_mutate:
        ...
        # INTELLIGENT NUTRIENT-BASED FILTERING
        # Skip if pure_random mode
        
        if not use_pure_random and need_carb and 'carbohydrate_g' in candidate.columns and 'protein_g' in candidate.columns:
            # NEED CARBS: Seek high-carb, moderate-protein foods
            high_carb_balanced = candidate[
                (candidate['carbohydrate_g'] >= 20) &  # High carb
                (candidate['protein_g'] <= 15)          # Moderate protein
            ]
            if len(high_carb_balanced) > 0:
                candidate = high_carb_balanced
            else:
        ...
        # Select new food from candidate with fallback strategy
        if len(candidate) > 0:
            selected_row = candidate.sample(n=1)
        elif len(slot_filtered) > 0:
            selected_row = slot_filtered.sample(n=1)
        else:
            selected_row = food_df.sample(n=1)
        
        # Get positional index in food_df
        food_idx_label = selected_row.index[0]  # Label-based index
        food_idx_pos = food_df.index.get_loc(food_idx_label)  # Positional index
        
        result[gene_idx] = food_idx_pos
    
    return result.astype(np.int64)


# ═════════════════════════════════════════════════════════════════════════════
# 5.5 NUMPY-BASED REPRESENTATION - Fast lookup with pre-built matrix
# ═════════════════════════════════════════════════════════════════════════════

def build_food_matrix(food_df: pd.DataFrame) -> Tuple[np.ndarray, List[str], pd.DataFrame]:
    """
    Pre-build nutrient matrix for fast numpy lookup (NUMPY OPTIMIZATION)
    
    Creates a single-pass lookup structure that enables individuals to be represented
    as arrays of food indices rather than DataFrames.
    
    Args:
        food_df: DataFrame containing all food items with nutrient columns
    
    Returns:
        Tuple of:
        - food_matrix: numpy array, shape (n_foods, n_nutrients), dtype float64
                      Contains nutrient values per food item for fast lookup
        - nutrient_cols: List[str] of nutrient column names (in same order as food_matrix columns)
        - food_df_reset: food_df with reset index for consistent indexing with food_matrix
    
    Example:
        >>> food_matrix, nutrient_cols, food_df_clean = build_food_matrix(food_df)
        >>> # Later in GA:
        >>> indices = np.array([0, 5, 12, 3, 8, 15, 2, 7, 10, 4])  # 10 food indices
        >>> nutrition = calculate_total_nutrition_numpy(indices, food_matrix, nutrient_cols)
    
    Performance:
    - ONE-TIME: O(n_foods) to build matrix
    - PER EVALUATION: O(10) instead of O(10) DataFrame operations
    """
    # Extract numeric columns (nutrients only)
    ...
    return food_matrix, nutrient_cols, food_df_reset

def _compute_grams_numpy(
    selected_nutrients: np.ndarray,
    nutrient_cols: List[str],
    tdee: float
) -> np.ndarray:
    """
    Hitung gram per slot berdasarkan target energy ratio dan TDEE.
    Satu-satunya sumber kebenaran untuk portioning logic.
    Dipanggil oleh calculate_total_nutrition_numpy() dan indices_to_dataframe().
    """
...
def calculate_total_nutrition_numpy(
    ...
    """
    Sum nutrients for 10 selected food indices from pre-built matrix (NUMPY OPTIMIZATION)
    Supports dynamic portion calculation (portioned nutrition) if tdee is provided.
    
    Args:
        indices: Array of 10 integers (food indices into food_matrix)
                 Shape: (10,), dtype int
        food_matrix: Pre-built nutrient matrix from build_food_matrix()
                     Shape: (n_foods, n_nutrients), dtype float64
        nutrient_cols: Column names list from build_food_matrix()
        tdee: Target Daily Energy Expenditure (kcal) for portion scaling
    """
    selected_nutrients = food_matrix[indices] # Shape: (10, n_nutrients)
    ...
    if tdee is not None and tdee > 0:
        ...
        scaled_nutrients = selected_nutrients * multipliers[:, np.newaxis]
        summed_nutrients = scaled_nutrients.sum(axis=0)
    else:
        # Standard 100g basis
        summed_nutrients = selected_nutrients.sum(axis=0)
    
    total_nutrition = dict(zip(nutrient_cols, summed_nutrients))
    return total_nutrition
...
def indices_to_dataframe(
    indices: np.ndarray,
    food_df_reset: pd.DataFrame,
    tdee: Optional[float] = None,
    food_matrix: Optional[np.ndarray] = None,
    nutrient_cols: Optional[List[str]] = None
) -> pd.DataFrame:
    """
    Convert numpy index array to DataFrame solution, portioning it if tdee is provided.
    
    Args:
        indices: np.ndarray shape (10,) of food indices
        food_df_reset: DataFrame with reset index
        tdee: Target Daily Energy Expenditure (kcal) for portion scaling
        food_matrix: Pre-built nutrient matrix
        nutrient_cols: Column names list
    
    Returns:
        pd.DataFrame: 10 rows (one per food item)
    """
    from typing import cast
    result = food_df_reset.iloc[indices.astype(int)].copy().reset_index(drop=True)
    
    if tdee is not None and tdee > 0 and food_matrix is not None and nutrient_cols is not None:
        selected_nutrients = food_matrix[indices]
        clamped_grams = _compute_grams_numpy(selected_nutrients, nutrient_cols, tdee)
        ...
        for col in result.columns:
            if col.endswith(('_mg', '_g', '_mcg', '_iu', '_kcal')) or col in ['energy_kcal', 'protein_g', 'carbohydrate_g', 'fat_g']:
                result[f'final_{col}'] = result[col]
...
def run_ga_numpy(
    ...
    Same GA logic as run_ga() but with numpy-based individuals for speed.
    Individuals are np.ndarray shape (10,) of food indices, not DataFrames.
    
    Args:
        food_df: DataFrame semua food items
        guidelines: Dict constraints dari NutritionService
        tdee: Target daily energy expenditure (kcal) - CRITICAL HARD constraint
        generations: Jumlah generasi (default 70)
        pop_size: Ukuran populasi (default 110)
        elite_ratio: Fraksi elite untuk breeding (default 0.10 = top 10%)
        mutation_rate: Probability mutasi (default 0.25 = 25%)
        verbose: Print progress? (default True)
        deadline: Optional time limit in seconds
    
    Returns:
        Tuple: (best_solution_df, top_solutions_df_list)
               Identical format to run_ga() - both DataFrames
    
    Performance:
    - ~10-100x faster than run_ga() due to O(10) fitness evaluation
    - Uses pre-built food_matrix for instant nutrition lookup
    - Numpy array operations instead of DataFrame concatenation
    """
    
    global _filter_cache
    _filter_cache = {}  # Reset cache tiap request baru
    
    import time as time_module
    
    start_time = time_module.time() if deadline else None
    
    # Build food matrix once (one-time O(n_foods) cost)
    food_matrix, nutrient_cols, food_df_reset = build_food_matrix(food_df)
    ...
    # 50% guided, 50% random
    num_guided = pop_size // 2
    num_random = pop_size - num_guided
    ...
    for generation in range(generations):
        # Check deadline
        if deadline and start_time:
            elapsed = time_module.time() - start_time
            if elapsed > deadline:
                if verbose:
                    print(f"Deadline reached at generation {generation}")
                break
        
        # Evaluate population
        fitness_scores = []
        for individual in population:
            score = fitness_numpy(individual, food_matrix, nutrient_cols, guidelines, tdee)
            fitness_scores.append(score)
        ...
        best_gen_fitness = sorted_scores[0]
        best_gen_individual = sorted_population[0]
        ...
        if verbose and (generation % 10 == 0 or generation == generations - 1):
            print(f"Gen {generation:3d}: best_fitness={best_gen_fitness:.2f}, "
                  f"avg_fitness={np.mean(sorted_scores):.2f}, "
                  f"worst_fitness={sorted_scores[-1]:.2f}")
        
        # ════════════════════════════════════════════════════════════════════
        # RESTART if converged too early (fitness still very high)
        # ════════════════════════════════════════════════════════════════════
        if generation == int(generations * 0.8) and best_gen_fitness > 1000:
            if verbose:
        ...
        # Select elite
        num_elite = max(1, int(pop_size * elite_ratio))
        ...
        while len(new_population) < pop_size:
            # Random select 2 parents from elite
            # LIMITASI: Semua offspring dihasilkan dari elite population saja.
        ...
        # Trim to pop_size if exceeded (due to elite + breeding)
        population = new_population[:pop_size]
        
        # ════════════════════════════════════════════════════════════════════
        # RANDOM INJECTION (every 10 generations)
        # ════════════════════════════════════════════════════════════════════
        
        if (generation + 1) % 10 == 0 and generation < generations - 1:
            ...
            for _ in range(min(2, num_to_replace)):
                if random.random() < 0.5:
                    ...
                    new_ind = random_solution_numpy(food_df_reset)
                population.append(new_ind)
    
    # ════════════════════════════════════════════════════════════════════════
    # FINAL EVALUATION - Convert to DataFrame
    # ════════════════════════════════════════════════════════════════════════
    
    if verbose:
        print(f"\n{'='*50}")
        print(f"GA Complete: {len(fitness_history)} generations")
        print(f"Best fitness: {best_fitness:.2f}")
        print(f"{'='*50}\n")
    
    # Convert best individual to DataFrame
    best_solution_df_pre = indices_to_dataframe(best_individual, food_df_reset, tdee, food_matrix, nutrient_cols)
    best_solution = local_search(
        solution=best_solution_df_pre,
        food_df=food_df,
        guidelines=guidelines,
        tdee=tdee,
        iterations=30,
        ...
        food_matrix=food_matrix,
        nutrient_cols=nutrient_cols,
        food_df_reset=food_df_reset
)    
    # Final evaluation of entire population
    final_fitness_scores = []
    for individual in population:
        score = fitness_numpy(individual, food_matrix, nutrient_cols, guidelines, tdee)
        final_fitness_scores.append(score)
    ...
    top_solutions_df = [indices_to_dataframe(ind, food_df_reset, tdee, food_matrix, nutrient_cols) for ind in top_solutions]
...
def calculate_total_hard_deviation(solution: pd.DataFrame, guidelines: Dict) -> float:
    ...
    """
    # Selalu pakai final_* columns (hasil portioning aktual)
    if 'final_energy_kcal' in solution.columns:
        total_nutrition = calculate_total_nutrition_from_portions(solution)
    else:
        # Fallback: harus ada final_* columns, kalau tidak ada berarti ada bug di upstream
        raise ValueError("validate_final_solution dipanggil sebelum portioning selesai. "
                        "Pastikan indices_to_dataframe dipanggil dengan food_matrix dan nutrient_cols.")
...
def local_search(
    ...
    food_matrix: Optional[np.ndarray] = None,
    nutrient_cols: Optional[List[str]] = None,
    food_df_reset: Optional[pd.DataFrame] = None
) -> pd.DataFrame:
    """
    ...
    if verbose:
        print(f"\n{'='*70}")
        print(f"LOCAL SEARCH - FAST NUMPY")
        print(f"{'='*70}")
    
    if food_matrix is None or nutrient_cols is None or food_df_reset is None:
        food_matrix, nutrient_cols, food_df_reset = build_food_matrix(food_df)
    
    # Pakai fdc_id sebagai key, bukan food_name — hindari ambiguitas nama duplikat
    food_id_to_idx = {
        fdc_id: idx 
        for idx, fdc_id in enumerate(food_df_reset['fdc_id'])
    }
    
    def _solution_to_indices(solution_df: pd.DataFrame, food_df_reset: pd.DataFrame) -> np.ndarray:
        indices = []
        for i in range(len(solution_df)):
            fdc_id = solution_df.iloc[i].get('fdc_id', None)
            idx = food_id_to_idx.get(fdc_id)
            if idx is not None:
                indices.append(idx)
            else:
                print(f"[WARNING] Food ID '{fdc_id}' not found in lookup dict, fallback to index 0")
                indices.append(0)
        return np.array(indices, dtype=np.int64)

    best_indices = _solution_to_indices(solution, food_df_reset)
    ...
    hard_constraints = guidelines.get('hard', {})
    current_best_fitness = fitness_numpy(best_indices, food_matrix, nutrient_cols, guidelines, tdee)
    ...
    while iteration < iterations:
        ...
        current_nutrition = calculate_total_nutrition_numpy(best_indices, food_matrix, nutrient_cols, tdee)
        ...
        for gene_idx in range(CHROMOSOME_SIZE):
            ...
            current_food_idx = best_indices[gene_idx]
            current_food_series = food_df_reset.iloc[current_food_idx]
            current_value = current_food_series.get(target_nutrient, 0) or 0
            ...
            for idx_cand in range(min(15, len(slot_candidates))):
                ...
                fdc_id = new_food.get('fdc_id', None)
                new_food_idx_pos = food_id_to_idx.get(fdc_id)
                if new_food_idx_pos is None:
                    continue
                
                test_indices = best_indices.copy()
                test_indices[gene_idx] = new_food_idx_pos
                
                test_nutrition = calculate_total_nutrition_numpy(test_indices, food_matrix, nutrient_cols, tdee)
                ...
                new_fitness = fitness_numpy(test_indices, food_matrix, nutrient_cols, guidelines, tdee)
                
                if new_fitness < best_new_fitness:
                    best_new_fitness = new_fitness
                    best_swap = {
                        'gene_idx': gene_idx,
                        'current_food': current_food_series,
                        'new_food': new_food,
        ...
        if best_swap is not None and best_swap['improvement'] > 0:
            best_indices = best_swap['test_indices'].copy()
            current_best_fitness = best_swap['new_fitness']
            improvements += 1
            no_improvement_count = 0
            ...
            adaptive_stop_threshold = max(5, iterations // 6)
            if no_improvement_count >= adaptive_stop_threshold:
                if verbose:
                    print(f"\n[STOP] No improvement for {no_improvement_count} consecutive iterations")
                break
    
    best_solution = indices_to_dataframe(best_indices, food_df_reset, tdee, food_matrix, nutrient_cols)
    
    if verbose:
        final_fitness = fitness_numpy(best_indices, food_matrix, nutrient_cols, guidelines, tdee)
        print(f"\n{'='*70}")
...
# LEGACY - Tidak dipanggil di jalur produksi (run_ga_numpy).
# Portioning aktual dilakukan oleh _compute_grams_numpy() via indices_to_dataframe().
# Dipertahankan untuk referensi dan kemungkinan pengembangan di masa depan.
...
def display_portion_summary_dynamic(portion_df: pd.DataFrame, guidelines: Dict, TDEE: float):
    ...
    """
    # Distribusi porsi mengacu pada _compute_grams_numpy():
    # Target energy ratio per slot: Breakfast 23.75%, Lunch 33.75%,
    # Dinner 28.75%, Snack 13.75% dari TDEE.
    # Detail perhitungan gram per slot lihat fungsi _compute_grams_numpy().
