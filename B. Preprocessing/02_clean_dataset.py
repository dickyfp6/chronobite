...
"""
Script 02: Clean Dataset
Tujuan: Membersihkan dataset dengan beberapa tahapan:
1. Menambahkan label kategori (Main Course, Snack, Drink, Side Dish) dengan RuleBasedClassifier
2. Menghapus makanan haram (alkohol, babi, dll)
3. Menghapus kategori tidak relevan (Baby Foods, Spices, dll)
4. Memfilter berdasarkan kualitas nutrisi (protein>0, karbo>0, lemak>0)
5. Membersihkan data dari bahan mentah (raw/dry/powder) yang tidak siap konsumsi
6. Membuang makanan cepat saji dengan sodium/kolesterol sangat tinggi, serta blacklist spesifik.
"""

CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
INPUT_FILE = os.path.normpath(os.path.join(CURRENT_DIR, "..", "A. Data", "Data Processed", "01_base_dataset.csv"))
OUTPUT_FILE = os.path.normpath(os.path.join(CURRENT_DIR, "..", "A. Data", "Data Processed", "02_cleaned_dataset.csv"))

def main():
    print("="*60)
    print("STEP 02: Clean Dataset (Halal, Kategori, Raw Foods)")
    print("="*60)
    ...
    print(f"[1] Dataset awal: {len(df)} baris")

    # ==========================================
    # 1. APPLY RULE-BASED CLASSIFIER
    # ==========================================
    print("\n[2] Menjalankan Rule-Based Classifier untuk consumption_label...")
    rule_clf = RuleBasedFoodClassifier()
    df["consumption_label"] = rule_clf.predict(df)
    
    # Generic cuisine label
    df["cuisine_label"] = "Generic"
    print("  [OK] Label ter-assign")

    # ==========================================
    # 2. FILTER MAKANAN HARAM
    # ==========================================
    print("\n[3] Memfilter Makanan Haram...")
    haram_keywords = [
    ...
    before = len(df)
    df = df[~df["food_name"].str.upper().str.contains(pattern, na=False, regex=True)]
    print(f"  [FILTER] Makanan haram dihapus: {before - len(df)} item")

    # ==========================================
    # 3. HAPUS KATEGORI TERTENTU
    # ==========================================
    print("\n[4] Menghapus Kategori Makanan Tertentu...")
    categories_to_remove = ["Alcoholic Beverages", "Baby Foods", "Pork Products", "Spices and Herbs", "American Indian/Alaska Native Foods"]

    before = len(df)
    df = df[~df["food_group"].isin(categories_to_remove)]
    print(f"  [FILTER] Kategori dihapus: {before - len(df)} item")

    # Filter liver sausage
    pork_liver_keywords = ['braunschweiger', 'liver sausage', 'liverwurst', 'liver cheese', 'pate, liver']
    pork_liver_mask = (
        (df['food_group'] == 'Sausages and Luncheon Meats') &
        (df['food_name'].str.lower().str.contains('|'.join(pork_liver_keywords), na=False))
    )
    before = len(df)
    df = df[~pork_liver_mask]
    print(f"  [FILTER] Pork liver sausage/pate dihapus: {before - len(df)} item")

    # Filter Nutrisi Utama > 0
    before = len(df)
    df = df[
        (df["protein_g"] > 0) &
        (df["carbohydrate_g"] > 0) &
    ...
    print(f"  [FILTER] Nutrisi (protein>0, carbo>0, fat>0): {before - len(df)} item dihapus")

    # ==========================================
    # 4. ADVANCED CLEANING (Raw/Dry/Fast Food)
    # ==========================================
    print("\n[5] Advanced Cleaning (Bahan mentah, Junk Food, Blacklist)...")
    df_cleaned = df.copy()
    ...
    print(f"\n[OK] Selesai. Dataset tersimpan di: {OUTPUT_FILE}")
    print("="*60)
