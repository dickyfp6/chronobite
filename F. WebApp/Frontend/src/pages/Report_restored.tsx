...
const mealThemes = {
  ...
  snack: {
    ...
    glowClass: "absolute -top-16 -right-16 w-64 h-64 bg-gradient-to-br from-rose-500/40 via-pink-450/20 to-transparent
...
export function Report({ userData, onRegisterDownloadPDF }: ReportProps) {
  ...
  const [loggedWater, setLoggedWater] = useState<number>(() => {
    const saved = sessionStorage.getItem('dss_logged_water');
  ...
  const totalWaterLiters = useMemo(() => {
    return foodWaterLiters + (loggedWater / 1000);
  }, [foodWaterLiters, loggedWater]);
  ...
  const nutritionalWarnings = useMemo(() => {
    ...
    const checkNutrient = (key: string, name: string, actualValue: number, minVal: number | null, maxVal: number | null, unit: string) => {
      if (minVal !== null && actualValue < minVal) {
        ...
        const msg = `${name} Deficiency: You need an additional ${diff}${unit} to meet the minimum target of ${Math.round(minVal)}${unit}.`;
        ...
        >
                    ...
                    {getMealItems('breakfast').map((item, i) => (
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="font-bold text-gray-900 dar
          ...
          {activeTab === 2 && (
            <div>
                                ...
                                <polygon
                                  ...
                                  <div className="flex flex-wrap gap-2 pt-2">
                                    <button
                                      ...
                                      className="px-3 py-1.5 text-[11px] font-bold bg-sky-500/10 hover:bg-sky-500/20 text-sky-700 dark:text-sky-350 dark:bg-sky-500/15 dark:hover:bg-sky-500/25 border border-sky-500/20 rounded-xl transition-all cursor-pointer shadow-sm active:scale-95"
                                    >
                                      +250 ml
                                    </button>
                              {/* Glass Top Rim */}
                ...
                return (
                  <>
                    ...
                    <div className="mb-8">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 relative z-
                        {items.map((item, i) => {
              ...
              {nutritionalWarnings.length > 0 && (() => {
                const deficiencies = nutritionalWarnings.filter(w => w.type === 'deficient');
                const excesses = nutritionalWarnings.filter(w => w.type === 'excessive');
                    ...
                    {deficiencies.length > 0 && (
                      <div className="p-6 rounded-2xl border bg-red-500/5 dark:bg-red-550/10 border-red-500/15 dark:border-red-500/10 shadow-sm font-sans animate-fade-in">
                        <div className="flex items-center gap-2.5 mb-3">
                          ...
                          <h3 className="font-bold text-sm text-red-950 dark:text-red-300 font-serif">
                            Nutrient Deficiencies (Kekurangan Asupan)
                          </h3>
                        </div>
                        <p className="text-xs text-gray-600 dark:text-gray-400 mb-4 font-sans leading-relaxed">
                          The following mandatory nutritional targets are still below the minimum required limit:
                        <span className="flex-1 leading-relaxed">{warning.message}</span>
                      </li>
                    ))}
                  </ul>
              ...
              <div className="mb-4 flex flex-wrap items-center gap-6 text-sm">
                <div className="flex items-center gap-2">
                        <div className="flex items-center gap-2.5 mb-3">
                          ...
                          <h3 className="font-bold text-sm text-amber-955 dark:text-amber-300 font-serif">
                            Nutrient Excesses (Kelebihan Asupan)
                          </h3>
                        </div>
                        <p className="text-xs text-gray-600 dark:text-gray-400 mb-4 font-sans leading-relaxed">
                          The following nutritional values exceed the maximum safety limit:
                        </p>
                        <ul className="space-y-2.5">
                          {excesses.map((warning, index) => (
                            <li key={index} className="flex items-start gap-2.5 text-xs text-amber-900 dark:text-amber-400 font-medium">
              ...
              {microData.length > 0 && (
                <div className="mt-8">
                  ...
                  if (nutrientKey === 'water_g') {
                    // Include logged water in the total actual intake
                    actual = Math.round(actual + loggedWater);
