...
export function ProfileSummary({ userData, onBack, onContinue, onAnalysisComplete }: ProfileSummaryProps) {
 ...
 {guideline}
  ...
  <div className="flex flex-row gap-2.5 sm:gap-3 justify-between mt-8">
  <button
  onClick={onBack}
  className="flex-1 sm:flex-none px-4 py-2.5 justify-center rounded-2xl font-semibold transition-all hover:bg-secondary :bg-slate-800 border border-border text-gray-900 flex items-center justify-center gap-2 cursor-pointer text-xs sm:text-sm"
  >
  <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5" />
  Back<span className="hidden sm:inline"> to Profile Input</span>
  </button>

  <button
  onClick={onContinue}
  disabled={loading}
  className="flex-1 sm:flex-none px-4 py-2.5 justify-center bg-primary text-primary-foreground rounded-2xl font-semibold hover:bg-primary/95 transition-all flex items-center justify-center gap-2 shadow-md hover:shadow-lg hover:shadow-primary/10 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer text-xs sm:text-sm"
  >
  Next
