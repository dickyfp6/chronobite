...
function SidebarNutritionSummary({ selectedItems, analysisResult }: { selectedItems: Record<string, any>, analysisResult: any }) {
 ...
 if (actual > range.max) return 'bg-[#a63a3a] #c94f4f]'; // classy brick red for over limit
 return 'bg-primary '; // forest green to match calorie target
 };
 ...
 <span className="flex items-center gap-1.5 text-gray-600 font-sans">
 <Flame className="w-3.5 h-3.5 text-orange-500" />
 {t.results.dailyCalories}
 </span>
 <span className="font-bold text-gray-900 ">{Math.round(totalCalories)} / {targetCalories}</span>
 </div>
 ...
 <span className="font-bold text-gray-900 ">{Math.round(totalCarbs)} / {Math.round(targetCarbs)}g</span>
 </div>
 ...
 <span className="font-bold text-gray-900 ">{Math.round(totalProtein)} / {Math.round(targetProtein)}g</span>
 </div>
 <div className="w-full bg-secondary h-1.5 rounded-full overflow-hidden">
 ...
 <span className="font-bold text-gray-900 ">{Math.round(totalFat)} / {Math.round(targetFat)}g</span>
 </div>
 <div className="w-full bg-secondary h-1.5 rounded-full overflow-hidden">
...
function SidebarDemographics({ userData }: { userData: UserInputData }) {
 return (
 <div className="hidden lg:block mt-6 pt-6 border-t border-border/80 space-y-4">
 <h3 className="font-bold text-sm text-gray-900 mb-4 font-serif">Demographics</h3>
 
 <div>
 <span className="text-[10px] font-bold text-primary uppercase tracking-wider block mb-0.5">Gender & Age</span>
 <p className="text-sm font-bold text-gray-900 capitalize font-sans">
 {userData.gender === 'male' ? 'Male' : 'Female'}, {userData.age} yrs
 </p>
 </div>

 <div>
 <span className="text-[10px] font-bold text-primary uppercase tracking-wider block mb-0.5">Weight & Height</span>
 ...
 <div>
 <span className="text-[10px] font-bold text-primary uppercase tracking-wider block mb-0.5">Activity Level</span>
 <p className="text-sm font-bold text-gray-900 capitalize font-sans">
 ...
 <div>
 <span className="text-[10px] font-bold text-primary uppercase tracking-wider block mb-0.5">Food Preferences</span>
 <p className="text-sm font-bold text-gray-900 capitalize font-sans line-clamp-2" title={userData.foodPreferences.join(', ')}>
...
export default function App() {
 ...
 <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl border border-border/80 ">
 ...
 className="px-6 py-2.5 rounded-2xl border border-gray-200 text-gray-700 hover:bg-gray-50 :bg-slate-700 transition-all font-sans font-medium text-sm cursor-pointer"
 >
 Cancel
 </button>
 <button
