...
export async function generateNutritionPDF(data: PDFData): Promise<void> {
  ...
  pdf.text('NutriPlan', margin, 20);
  pdf.setFontSize(12);
  pdf.setFont('helvetica', 'normal');
  pdf.text(data.language === 'en' ? 'Personalized Nutrition Report' : 'Laporan Nutrisi Personal', margin, 30);

  // Date
  pdf.setFontSize(10);
  const today = new Date().toLocaleDateString(data.language === 'en' ? 'en-US' : 'id-ID', {
    year: 'numeric',
    month: 'long',
    day: 'numeric'
  });
  ...
  pdf.text(
    data.language === 'en'
      ? 'Generated by NutriPlan - Your Personal Nutrition Assistant'
      : 'Dibuat oleh NutriPlan - Asisten Nutrisi Personal Anda',
    pageWidth / 2,
    pageHeight - 10,
    { align: 'center' }
  );
